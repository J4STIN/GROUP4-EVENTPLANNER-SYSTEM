...
class EventPlannerGUI:
    ...
    # --- DATABASE METHODS (From your original eventplanner.py) ---
    ...
    def setup_style(self):
        ...
        style.map(
            "TNotebook.Tab",
            background=[
                ("selected", self.colors["surface_soft"]),
        ...
        style.map(
            "Treeview.Heading", background=[("active", self.colors["border"])]
        )
        style.configure(
            "Primary.TButton",
        ...
        style.map(
            "Primary.TButton", background=[("active", self.colors["accent"])]
        )
        style.configure(
            "TButton",
            background=self.colors["surface_soft"],
        ...
        style.configure(
            "TLabel", background=self.colors["bg"], foreground=self.colors["muted"], font=("Arial", 10)
        )
        style.configure(
            "Panel.TLabel",
            background=self.colors["surface"],
    ...
    def build_layout(self):
        # Header Application Brand Frame
        ...
        ttk.Label(
            title_area, text="Event Planner System", style="Title.TLabel"
        ).pack(anchor="w")
        ttk.Label(
            title_area,
            text="MEMBER: JOHN PAUL CASCARA, DAVE KEVIN CABAB, JUSTIN PAUL GUASIL, GERRY ESPERAS, LORENZ TUBIS, KYLE LESTER NAVAL",
    ...
    def build_crud_tab(self):
        # Entry Input Form Frame
        form = ttk.Frame(self.crud_tab, padding=16, style="Panel.TFrame")
        form.pack(fill="x", pady=(10, 0))

        ttk.Label(form, text="Event ID", style="Panel.TLabel").grid(
            row=0, column=0, sticky="w"
        )
        ttk.Label(form, text="Event Name", style="Panel.TLabel").grid(
            row=0, column=1, sticky="w", padx=(12, 0)
        )
        ttk.Label(form, text="Event Date", style="Panel.TLabel").grid(
            row=0, column=2, sticky="w", padx=(12, 0)
        )
        ttk.Label(form, text="Venue Location", style="Panel.TLabel").grid(
            row=2, column=0, sticky="w", pady=(10, 0)
        )
        ttk.Label(form, text="Organizer Name", style="Panel.TLabel").grid(
        ...
        self.date_entry.grid(
            row=1, column=2, sticky="ew", padx=(12, 0), pady=(4, 0)
        )
        self.venue_entry.grid(row=3, column=0, sticky="ew", pady=(4, 0))
        self.organizer_entry.grid(
            row=3, column=1, columnspan=2, sticky="ew", padx=(12, 0), pady=(4, 0)
        )

        # Control Operation Buttons Layout
        ...
        ttk.Button(
            buttons, text="Update Details", command=self.update_event
        ).pack(side="left", padx=6)
        ttk.Button(buttons, text="Delete Event", command=self.delete_event).pack(
            side="left"
        )
        ttk.Button(buttons, text="Clear Entries", command=self.clear_form).pack(
            side="left", padx=6
        )
        ...
        # Interactive Table for selecting data
        search_frame = ttk.Frame(self.crud_tab, padding=(14, 12, 14, 6))
        search_frame.pack(fill="x")
        ttk.Label(
            search_frame, text="Select an event from the list below to edit/delete:"
        ).pack(side="left")

        columns = ("id", "name", "date", "venue", "organizer")
        self.events_tree = ttk.Treeview(
            self.crud_tab, columns=columns, show="headings", selectmode="browse"
        )
    ...
    def build_view_tab(self):
        # A clean layout alternative strictly for non-editable viewing representation
        records_frame = ttk.Frame(self.view_tab, padding=14)
        records_frame.pack(fill="both", expand=True)

        columns = ("id", "name", "date", "venue", "organizer")
        self.view_only_tree = ttk.Treeview(
            records_frame, columns=columns, show="headings", selectmode="none"
        )
    ...
    # --- CONTROLLER DATA & REFRESH METHODS ---
    ...
    def refresh_stats(self, events):
        ...
        unique_venues = len(set(event["Venue"] for event in events if event["Venue"]))

        self.stat_labels["total_events"].configure(text=str(total_events))
        self.stat_labels["unique_venues"].configure(text=str(unique_venues))
    ...
    def on_event_select(self, _event=None):
        ...
        events = self.load_events()
        event = next(
            (e for e in events if str(e["Event ID"]) == self.selected_event_id),
            None,
        )

        if event:
            # Populating administrative fields for updating or removing execution strings
            self.id_entry.delete(0, tk.END)
            self.id_entry.insert(0, str(event["Event ID"]))
            self.id_entry.configure(state="disabled")  # ID immutable lock
    ...
    def get_form_values(self):
        ...
        organizer = self.organizer_entry.get().strip()

        if not event_id or not name or not date or not venue or not organizer:
            raise ValueError("All interface structural fields are required.")
        return event_id, name, date, venue, organizer

    # --- UI INTERACTIVE LOGIC (CRUD ALTERNATIVES) ---
    ...
    def create_event(self):
        ...
        if any(str(e["Event ID"]) == str(event_id) for e in events):
            messagebox.showerror(
                "Duplicate Error",
                f"An event record matching ID '{event_id}' already exists.",
            )
            return
    ...
    def update_event(self):
        if self.selected_event_id is None:
            messagebox.showwarning(
                "Selection Missing",
                "Select an operational schedule entry from the structural grid to update.",
            )
            return
        ...
        self.clear_form()
        self.refresh_all()
        messagebox.showinfo("Success", "Event structural data altered.")

    def delete_event(self):
        if self.selected_event_id is None:
            messagebox.showwarning(
                "Selection Missing",
                "Select an operational schedule entry to delete.",
            )
            return

        events = self.load_events()
        event = next(
            (e for e in events if str(e["Event ID"]) == str(self.selected_event_id)),
            None,
        )

        if event and messagebox.askyesno(
            "Confirm Action", f"Delete event data: {event['Event Name']}?"
        ):
            events = [
                e
                for e in events
                if str(e["Event ID"]) != str(self.selected_event_id)
            ]
            self.save_events(events)
            self.clear_form()
            self.refresh_all()
